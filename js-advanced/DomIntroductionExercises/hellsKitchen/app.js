function solve() {
    ...
    function onClick() {
        ...
        function calcAverageSalary(salaries) {
            return (
                salaries.reduce((a, b) => Number(a) + Number(b)) /
                salaries.length
            );
        }
        ...
        function sortSalaries() {
            ...
            let sorted = Object.entries(workersAndSalaries).sort(
                (a, b) => b[1] - a[1]
            );
            print(sorted);
        }
        ...
        function print(sorted) {
            bestRestaurantOutput.textContent = `Name: ${
                restaurant[0]
            } Average Salary: ${restaurant[1].toFixed(
                2
            )} Best Salary: ${calcHighestSalary().toFixed(2)}`;
            sorted.forEach(
                ([name, salary]) =>
